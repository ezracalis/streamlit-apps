...
if st.button("Submit"):  # only display model predictions on UI if user clicks "Submit" button
    ...
    st.write(f"Sentiment: {predictions}")  # prediction values were stored in 'predictions' key of dict: predictions. [0] is to give prediction output 0/1 in a "unlisted" format since we're only sending user inputs for 1 row of X at a time
